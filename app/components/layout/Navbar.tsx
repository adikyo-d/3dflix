...
export default function Navbar() {
  ...
  // --- SESSION dari Auth.js ---
  const { data: session, status } = useSession();
  const isLoggedIn = status === "authenticated";
  const username = session?.user?.name ?? "";

  // Ambil role secara aman dengan type-casting custom property
  ...
  return (
    <nav
      className={`sticky top-0 z-50 transition-all duration-300 backdrop-blur-md ${
        scrolled
          ? "bg-[#14181c]/95 border-b border-[#2c3440] shadow-lg shadow-black/40"
          : "bg-[#14181c]/80 border-b border-[#2c3440]/40"
      }`}
    >
      <div className="max-w-6xl mx-auto px-4 h-15 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 relative z-50">
          ...
          <div className="hidden sm:flex items-center gap-2">
            ...
            {status === "loading" ? (
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-40 bg-[#2c3440] border border-[#455568] rounded-lg shadow-xl overflow-hidden z-50">
                    <Link
                      ...
                      className="block px-4 py-2.5 text-xs font-bold text-white hover:bg-[#40bcf4] hover:text-black"
                    >
                      Profile
                    </Link>
                    <Link
                    ...
                    {userRole === "admin" && (
                      <>
                        ...
                        <Link
                          href="/admin"
                          onClick={() => setIsDropdownOpen(false)}
                          className="block px-4 py-2.5 text-xs font-bold text-[#00e054] hover:bg-[#00e054] hover:text-black transition-colors"
                        >
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-xs font-bold text-red-400 hover:bg-red-500 hover:text-white"
                    >
                      Sign Out
                    </button>
                  </div>
              ...
              <>
                <Link
                  href="/login"
                  className="text-xs font-bold uppercase tracking-wider text-[#9ab] hover:text-white transition-colors"
                >
                  Sign In
                </Link>
                <Link
                  href="/register"
                  className="px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide bg-[#00e054] text-black hover:bg-[#00c04b] hover:-translate-y-px hover:shadow-[0_0_16px_rgba(0,224,84,0.4)] transition-all"
                >
                  Sign Up
                </Link>
              </>
          ...
          >
            <i
              className={`fa-solid ${isOpen ? "fa-xmark" : "fa-bars"} text-sm`}
            />
          </button>
        </div>
      </div>
        ...
        <div className="md:hidden border-t border-[#2c3440] p-4 bg-[#14181c]">
          <form onSubmit={handleSearch} className="relative mb-4">
            ...
            <input
              ...
              onChange={(e) => setSearchVal(e.target.value)}
              placeholder="Cari film..."
              className="w-full bg-[#1c2228] border border-[#2c3440] text-sm rounded-lg pl-8 pr-4 py-2.5 text-white focus:outline-none"
            />
          </form>
          ...
          <div className="flex flex-col gap-1">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                href={link.href}
                onClick={toggleMenu}
                className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase text-[#9ab] hover:text-white hover:bg-[#2c3440]/60"
              >
            ...
            {SECTION_LINKS.map((link) => (
              <button
                key={link.href}
                onClick={() => scrollToSection(link.href)}
                className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase text-left text-[#9ab] hover:text-[#00e054] hover:bg-[#00e054]/10"
              >
                <i className={`fa-solid ${link.icon} text-xs w-4`} />{" "}
                {link.label}
              </button>
            ...
            {isLoggedIn ? (
              <>
                <Link
                  href="/profile"
                  onClick={toggleMenu}
                  className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase text-white bg-[#2c3440]/60 sm:hidden"
                >
                  <i className="fa-solid fa-user text-xs w-4 text-[#00e054]" />{" "}
                  {username}
                </Link>

                {/* HAK AKSES ADMIN MOBILE */}
                {userRole === "admin" && (
                  <Link
                    href="/admin"
                    onClick={toggleMenu}
                    className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase text-[#00e054] bg-[#00e054]/10 sm:hidden"
                  >
                    <i className="fa-solid fa-shield-halved text-xs w-4" />{" "}
                    Admin Panel
                  </Link>
                )}
                ...
                <button
                  onClick={handleLogout}
                  className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase text-left text-red-400 sm:hidden"
                >
              ...
              <>
                <Link
                  href="/login"
                  onClick={toggleMenu}
                  className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase text-[#9ab] sm:hidden"
                >
                  <i className="fa-solid fa-right-to-bracket text-xs w-4" />{" "}
                  Sign In
                </Link>
                <Link
                  href="/register"
                  onClick={toggleMenu}
                  className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold uppercase bg-[#00e054] text-black hover:bg-[#00c04b] hover:-translate-y-px hover:shadow-[0_0_16px_rgba(0,224,84,0.4)] transition-all sm:hidden"
                >
                  <i className="fa-solid fa-user-plus text-xs w-4" /> Sign Up
                </Link>
              </>
            )}
          </div>
